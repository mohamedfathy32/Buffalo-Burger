...
export default function Bread({ selectedBread, onBreadChange }) {
    ...
    return (isBread &&
        <section className="w-full bg-stone-50 p-6" >
            ...
            <RadioGroup value={selectedBread || breadList[0].title.en} onChange={(e) => { onBreadChange(e.target.value) }} aria-labelledby="demo-radio-buttons-group-label" defaultValue={breadList[0].title.en} name="radio-buttons-group">
                <div className="flex flex-col gap-2 lg:gap-8 lg:flex-row justify-center items-center">
                    {breadList.map(bread =>
                        <div key={bread.title.en} className="bg-white rounded-xl capitalize lg:w-72 w-52 px-4 py-2">
                            <FormControlLabel value={bread.title.en} control={<Radio sx={radioStyles} />} sx={labelStyles}
                                label={`${i18n.language === 'en' ? bread.title.en : bread.title.ar} ${i18n.language === 'en' ? 'Bread' : ''}  ${bread.price === 0 ? '' : `(${i18n.language === 'en' ? 'EGP' : 'ج.م'} ${bread.price})`}`} />
                        </div>
                    )}
                </div>
            </RadioGroup>
