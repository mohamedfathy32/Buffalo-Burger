...
export default function Bread({ selectedBread, onBreadChange }) {
    ...
    return isBread &&
        <section className="w-full bg-stone-50 p-6">
            ...
            <RadioGroup value={selectedBread?.[i18n.language] || breadList[0].title[i18n.language]}
                onChange={(e) => { const selected = breadList.find(bread => bread.title[i18n.language] === e.target.value); onBreadChange(selected ? selected.title : {}); }}
                aria-labelledby="demo-radio-buttons-group-label" defaultValue={breadList[0].title[i18n.language]} name="radio-buttons-group">
